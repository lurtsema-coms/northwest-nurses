<div class="hidden min-h-dvh w-72 shrink-0 md:block" id="sidebar">
    ...
    <div class="py-10 pl-7" id="sidebar-links">
        <div class="space-y-3">
            ...
            <div 
                class="flex items-center w-full border shadow-sm py-2 px-3 rounded-xl text-slate-500 space-x-2 text-sm cursor-pointer hover:bg-slate-300 url-links {{ Route::is('admin.accounts') || Route::is('admin.accounts.show')  ? 'bg-slate-300' : 'bg-white'  }}"
                id="m-employee-link"
                hx-boost="true"
                hx-get="{{ route('admin.accounts') }}" hx-target="#target-content" hx-push-url="true" hx-on:click="removeBgColor(); addBgColorLink(event); $('#target-content').hide()" hx-on::after-request="$('#target-content').show()" hx-indicator="#htmx-indicator">
                <div class="p-1 rounded-lg bg-cyan-600">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="text-white size-5">
